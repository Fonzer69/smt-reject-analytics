...
st.set_page_config(page_title="Oradea SMT — Multi-Day Dashboard v5.5", layout="wide")

DATA_STORE = "smt_saved_data.parquet"
PRICE_STORE = "price_store.parquet"
CPK_COMPONENTS = {"CP1", "CC02-05_CPP", "CC02-05"}

st.title("📊 Oradea SMT — Multi-Day Line Reject Dashboard (v5.5)")
st.caption("""
UI simplificat pentru trend: opțiune unică pentru linia **Factory (All Lines)** care unește toate liniile (ignoră filtrul de linii). 
Păstrează etichetele % (2 zecimale), listele copiabile și restaurarea store‑ului.
""")
...
# Apply base filters (date + toggles that should affect everything)
...
# Keep a copy with ALL lines (no line filter) for the "Factory (All Lines)" overlay
all_lines_df = subset.copy()

# Apply line filter for the main views
if sel_lines:
...
# ---------------------- Trend — simplu + opțiune Factory (All Lines) ----------------------
...
# ---------------------- Totals by line — Placed vs Waste ----------------------
...
# Etichetă 2 zecimale deasupra (Wasted / Sum)
