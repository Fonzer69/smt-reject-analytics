...
st.set_page_config(page_title="Oradea SMT — Multi-Day Dashboard v5.2", layout="wide")

DATA_STORE = "smt_saved_data.parquet"
PRICE_STORE = "price_store.parquet"
CPK_COMPONENTS = {"CP1", "CC02-05_CPP", "CC02-05"}

st.title("📊 Oradea SMT — Multi-Day Line Reject Dashboard (v5.2)")
st.caption("""
New in v5.2: exclude **Label** components and show **waste % per line** above the waste bars.
""")
...
# Exclude 'Label' components (contains "label" anywhere)
if exclude_label:
...
# Add label only for Wasted bars
by_line_melt["Label"] = np.where(by_line_melt["Metric"] == "Wasted",
                                 by_line_melt["Waste %"].map(lambda x: f"{x:.1f}%" if pd.notna(x) else ""),
                                 "")

fig_byline = px.bar(by_line_melt, x="Line", y="Value", color="Metric", barmode="group", text="Label")
fig_byline.update_traces(textposition="outside")
