...
st.set_page_config(page_title="Oradea SMT — Multi-Day Dashboard v5.3", layout="wide")
...
# Add a 2-decimal percent label per line above the tallest bar (centered)
...
# ---------------------- Top by Wasted (pcs) — follows Top N slider ----------------------
st.subheader(f"Top {top_n} by Total Wasted (pcs) — Component")
top_wasted_qty = (
    subset.groupby("Component", dropna=False)["Wasted"]
    .sum()
    .reset_index()
    .sort_values("Wasted", ascending=False)
    .head(top_n)
)
fig_wasted_qty = px.bar(top_wasted_qty, x="Component", y="Wasted")
fig_wasted_qty.update_layout(xaxis_title="Component", yaxis_title="Wasted (pcs)")
st.plotly_chart(fig_wasted_qty, use_container_width=True)
# ---------------------- Copyable component lists ----------------------
