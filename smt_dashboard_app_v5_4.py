...
st.set_page_config(page_title="Oradea SMT — Multi-Day Dashboard v5.4", layout="wide")

DATA_STORE = "smt_saved_data.parquet"
PRICE_STORE = "price_store.parquet"
CPK_COMPONENTS = {"CP1", "CC02-05_CPP", "CC02-05"}

st.title("📊 Oradea SMT — Multi-Day Line Reject Dashboard (v5.4)")
st.caption("""
Adds **Factory (All Lines)** trend line (ignores the Line filter) and optional **Rest of Factory** (All − Selected). Keeps all v5.3 features.
""")
...
with st.sidebar:
    ...
    exclude_label = st.checkbox("Exclude 'Label' components", value=True)

# Apply base filters
mask_date = (pd.to_datetime(data["Report Date"]).dt.date >= start) & (pd.to_datetime(data["Report Date"]).dt.date <= end)
subset = data.loc[mask_date].copy()
# Apply toggles common to all
...
# Make a version with all lines (no line filter) for factory/all calculations
all_lines_df = subset.copy()

# Now apply line filter to create the per-line subset
if sel_lines:
    subset = subset[subset["Line"].isin(sel_lines)]
...
def add_period_col(df: pd.DataFrame, granularity: str) -> pd.DataFrame:
    df = df.copy()
    df["Report Date"] = pd.to_datetime(df["Report Date"])
    if granularity == "Day":
        df["Period"] = df["Report Date"].dt.date
    elif granularity == "Week":
        df["Period"] = df["Report Date"].dt.to_period("W-SUN").astype(str)
    else:
        df["Period"] = df["Report Date"].dt.to_period("M").astype(str)
    return df

subset = add_period_col(subset, granularity)
all_lines_df = add_period_col(all_lines_df, granularity)

# ---------------------- KPIs (based on selected lines) ----------------------
...
# ---------------------- Trend with factory lines ----------------------
st.subheader("Trend — Reject % over time (with Factory lines)")
...
show_factory_all = st.checkbox("Show **Factory (All Lines)**", value=True, help="Aggregates all lines, ignores the Line filter.")
show_rest_factory = st.checkbox("Show **Rest of Factory** (All − Selected)", value=False)

# Per-line trends (selected lines)
trend_line = subset.groupby(["Period","Line"], dropna=False).agg({"Sum":"sum","Wasted":"sum", "Rejected %": "mean"}).reset_index()

# Selected combined (Factory Overall of selected lines)
trend_selected = subset.groupby(["Period"], dropna=False).agg({"Sum":"sum","Wasted":"sum", "Rejected %":"mean"}).reset_index()
trend_selected["Line"] = "Factory Overall (Selected)"
trend_base = pd.concat([trend_line, trend_selected], ignore_index=True)

# Factory (all lines) — ignores line filter but keeps date/toggles
if show_factory_all:
    trend_all = all_lines_df.groupby(["Period"], dropna=False).agg({"Sum":"sum","Wasted":"sum", "Rejected %":"mean"}).reset_index()
    trend_all["Line"] = "Factory (All Lines)"
    trend_base = pd.concat([trend_base, trend_all], ignore_index=True)

# Rest of Factory = All - Selected
if show_rest_factory:
    if show_factory_all:
        left = trend_all.rename(columns={"Sum":"Sum_all","Wasted":"Wasted_all","Rejected %":"Rejected_all"})
    else:
        # compute anyway to get "all"
        tmp_all = all_lines_df.groupby(["Period"], dropna=False).agg({"Sum":"sum","Wasted":"sum", "Rejected %":"mean"}).reset_index()
        left = tmp_all.rename(columns={"Sum":"Sum_all","Wasted":"Wasted_all","Rejected %":"Rejected_all"})
    right = trend_selected.rename(columns={"Sum":"Sum_sel","Wasted":"Wasted_sel","Rejected %":"Rejected_sel"})
    merged = pd.merge(left[["Period","Sum_all","Wasted_all"]], right[["Period","Sum_sel","Wasted_sel"]], on="Period", how="left").fillna(0.0)
    merged["Sum"] = merged["Sum_all"] - merged["Sum_sel"]
    merged["Wasted"] = merged["Wasted_all"] - merged["Wasted_sel"]
    # avoid negative from numerical rounding
    merged["Sum"] = merged["Sum"].clip(lower=0)
    merged["Wasted"] = merged["Wasted"].clip(lower=0)
    merged["Rejected %"] = np.where(merged["Sum"]>0, (merged["Wasted"]/merged["Sum"])*100, np.nan)
    merged["Line"] = "Rest of Factory"
    trend_base = pd.concat([trend_base, merged[["Period","Line","Sum","Wasted","Rejected %"]]], ignore_index=True)
...
by_line["Waste %"] = np.where(by_line["Sum"].astype(float) > 0, (by_line["Wasted"].astype(float) / by_line["Sum"].astype(float)) * 100, np.nan)

by_line_melt = by_line.melt(id_vars=["Line","Waste %"], value_vars=["Sum","Wasted"], var_name="Metric", value_name="Value")
...
# Add centered 2-decimal label above tallest bar per line
for _, r in by_line.iterrows():
    ymax = float(max(r["Sum"] or 0, r["Wasted"] or 0))
    ytext = ymax * 1.03 if ymax > 0 else 0
    label = f'{r["Waste %"]:.2f}%' if pd.notna(r["Waste %"]) else ""
    fig_byline.add_annotation(x=r["Line"], y=ytext, text=label, showarrow=False, yanchor="bottom")

st.plotly_chart(fig_byline, use_container_width=True)

# ---------------------- Ranking charts + copyable lists ----------------------
...
top_reject = by_group.sort_values("Overall Reject %", ascending=False).head(top_n)
top_placed = subset.groupby(group_by, dropna=False)["Sum"].sum().reset_index().sort_values("Sum", ascending=False).head(top_n)
top_cost = None
if "Cost Wasted (USD)" in subset.columns:
    top_cost = subset.groupby(group_by, dropna=False)["Cost Wasted (USD)"].sum().reset_index().sort_values("Cost Wasted (USD)", ascending=False).head(top_n)

left, right = st.columns(2)
with left:
    st.subheader(f"Top {len(top_reject)} by Overall Reject % — {group_by}")
    fig1 = px.bar(top_reject, x=group_by, y="Overall Reject %", hover_data=["Sum","Wasted"] + ([ "Cost Wasted (USD)" ] if "Cost Wasted (USD)" in top_reject.columns else []))
...
with right:
    ...
    fig2 = px.bar(top_placed, x=group_by, y="Sum")
...
if group_by == "Component":
    st.subheader("Quick copy lists — Component names only")
    names_reject = top_reject["Component"].astype(str).tolist()
    names_placed = top_placed["Component"].astype(str).tolist()
    names_cost = top_cost["Component"].astype(str).tolist() if top_cost is not None else []
