...
st.set_page_config(page_title="Oradea SMT — Multi-Day Dashboard v5", layout="wide")

DATA_STORE = "smt_saved_data.parquet"
PRICE_STORE = "price_store.parquet"
CPK_COMPONENTS = {"CP1", "CC02-05_CPP", "CC02-05"}

st.title("📊 Oradea SMT — Multi-Day Line Reject Dashboard (v5)")
st.caption("""
Enhancements: exclude CPK components, persistent price list, factory-wide trendline, waste-over-time chart, and totals (placed & waste) by line.
""")
...
if "Cost Wasted (USD)" in subset.columns:
    ...
    cost_df = subset.groupby(group_by, dropna=False)["Cost Wasted (USD)"].sum().reset_index().sort_values("Cost Wasted (USD)", descending=True).head(top_n)
